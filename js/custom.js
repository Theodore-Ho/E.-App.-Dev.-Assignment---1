...
function generateCityInfo(city) {
	...
	play_video.attr("href", info.video);
	$("#video-cover-img").attr("src", "images/card/" + info.city + ".jpg");
	const wikipedia_url = $("#wikipedia-url");
	wikipedia_url.removeClass("placeholder");
	wikipedia_url.attr("href", info.wikipedia);
	const city_info_p = $("#city-info-paragraph");
	city_info_p.empty();
	...
	city_info_p.append(paragraph);
}

function generateCityInfoPlaceholder(city) {
	$("#city-name").text(city);
	const play_video = $("#play-video");
	play_video.removeAttr("href");
	play_video.css("display", "none");
	$("#video-cover-img").attr("src", "images/card/" + city + ".jpg");
	const wikipedia_url = $("#wikipedia-url");
	wikipedia_url.addClass("placeholder");
	wikipedia_url.removeAttr("href");
	const city_info_p = $("#city-info-paragraph");
	city_info_p.empty();
	let paragraph = $("<div></div>");
	for(let i = 0; i < 10; i ++) {
...
function owlPlugin() {
	const owl_single = $('.owl-single');
	if ( owl_single.length > 0 ) {
		const owl = owl_single.owlCarousel({
			loop: true,
			autoHeight: true,
			margin: 0,
			autoplay: true,
